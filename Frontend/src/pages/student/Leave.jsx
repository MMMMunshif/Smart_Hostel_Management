...
const LEAVE_TYPES = ["Temporary", "Permanent"];
...
const validate = (form) => {
  ...
  // To Date (required only for temporary leave)
  if (form.leaveType === "Temporary") {
    if (!form.toDate) {
...
function Leave() {
  ...
  const handleChange = (field, value) => {
    const updated =
      field === "leaveType" && value === "Permanent"
        ? { ...form, leaveType: value, toDate: "" }
        : { ...form, [field]: value };
    setForm(updated);
  ...
  const handleSubmit = async (e) => {
    ...
    setTouched({ leaveType: true, reason: true, fromDate: true, toDate: true });
    ...
    try {
      ...
      setForm({ leaveType: "Temporary", reason: "", fromDate: "", toDate: "" });
      setErrors({});
      setTouched({});
      fetchLeaves();
    } catch (err) {
          ...
          <div className="lv-form-card">
            ...
            <form onSubmit={handleSubmit} noValidate>
              ...
              <div className="lv-field">
                ...
                <select
                  className={`lv-field-input ${touched.leaveType && errors.leaveType ? "field-error" : ""}`}
                ...
                >
                  {LEAVE_TYPES.map((type) => (
                    <option key={type} value={type} style={{ color: "#1c1a17" }}>
                      {type}
                    </option>
                  ))}
                </select>
                {touched.leaveType && errors.leaveType && (
                  <span className="lv-field-error">âš  {errors.leaveType}</span>
                )}
              </div>
              ...
              <div className="lv-date-row">
                ...
                {form.leaveType === "Temporary" ? (
                  ...
                  <div className="lv-field">
                    <label className="lv-field-label">Permanent Leave Notice</label>
                    <div className="lv-field-input" style={{ display: "flex", alignItems: "center", minHeight: 45 }}>
                      End date is not required for permanent leave.
                    </div>
                  </div>
                )}
              </div>
