...
const formatLeaveWindow = (leave) => {
  if (!leave?.fromDate) return "N/A";

  const from = new Date(leave.fromDate).toDateString();
  if ((leave.leaveType || "Temporary") === "Permanent") {
    return `${from} (Permanent)`;
  }

  const to = leave.toDate ? new Date(leave.toDate).toDateString() : "N/A";
  return `${from} -> ${to}`;
};
